...
def load_tfidf_model():
    """Load TF-IDF + LogisticRegression model + keyword dictionaries."""
...
def predict_tfidf(text, model_data):
    ...
    model, vectorizer, keywords, harm_kws, harm_cats = model_data
    ...
    # Keyword features (must match training: 13 features)
    cat_names = ['sexual', 'self_harm', 'privacy', 'illegal', 'info_hazard', 'cultural', 'misinfo']
    n_features = 6 + len(cat_names)
    kw_feat = np.zeros((1, n_features))
    word_count = max(len(original_text.split()), 1)

    # Toxic keywords
    toxic_count = sum(1 for kw in keywords if kw in original_text)
    kw_feat[0, 0] = toxic_count
    kw_feat[0, 1] = toxic_count / word_count
    kw_feat[0, 2] = 1.0 if toxic_count > 0 else 0.0

    # Harm intent keywords
    harm_count = sum(1 for kw in harm_kws if kw in original_text)
    kw_feat[0, 3] = harm_count
    kw_feat[0, 4] = harm_count / word_count
    kw_feat[0, 5] = 1.0 if harm_count > 0 else 0.0

    for j, cat in enumerate(cat_names):
        if cat in harm_cats:
            kw_feat[0, 6 + j] = 1.0 if any(kw in original_text for kw in harm_cats[cat]) else 0.0

    X = hstack([X_tfidf, csr_matrix(kw_feat)])

    pred = model.predict(X)[0]
    proba = model.predict_proba(X)[0]

    # Safety net: if text contains toxic or harm keywords but model is uncertain
    all_matched = [kw for kw in keywords if kw in original_text] + \
                  [kw for kw in harm_kws if kw in original_text]
    if all_matched and proba[1] < 0.6:
        boost = min(0.95, proba[1] + 0.3 * len(all_matched))
