    ...
    if kw_path.exists():
        with open(str(kw_path), "r", encoding="utf-8-sig") as f:
            ...
            for row in reader:
                ...
                if kw:
                    keywords.append(kw)
        print(f"  Loaded {len(keywords)} toxic keywords")
    else:
        print(f"  [WARN] No toxic keyword dictionary found at {kw_path}")
    return keywords


def load_harm_intent_keywords():
    """Load harm intent keywords grouped by category."""
    ...
    if hi_path.exists():
        ...
        print(f"  Loaded {len(all_keywords)} harm-intent keywords across {len(categories)} categories")
        for cat, kws in categories.items():
            print(f"    {cat}: {len(kws)} keywords")
    return all_keywords, categories


def keyword_features(texts, toxic_kws, harm_kws=None, harm_cats=None):
    """
    Build keyword-based features for each text.

    Features:
      - toxic_count, toxic_ratio, has_toxic (3)
      - harm_count, harm_ratio, has_harm (3)
      - per-category flags: sexual, self_harm, privacy, illegal, info_hazard,
        cultural, misinfo (7)
    Total: 13 keyword features
    """
    import numpy as np
    from scipy.sparse import csr_matrix

    cat_names = ['sexual', 'self_harm', 'privacy', 'illegal', 'info_hazard', 'cultural', 'misinfo']
    n_features = 6 + len(cat_names)  # 13
    ...
    for i, text in enumerate(texts):
        ...
        toxic_count = sum(1 for kw in toxic_kws if kw in text)
        features[i, 0] = toxic_count
        features[i, 1] = toxic_count / word_count
        features[i, 2] = 1.0 if toxic_count > 0 else 0.0

        # Harm intent keyword features
        if harm_kws:
            harm_count = sum(1 for kw in harm_kws if kw in text)
            features[i, 3] = harm_count
            features[i, 4] = harm_count / word_count
            features[i, 5] = 1.0 if harm_count > 0 else 0.0

        # Per-category flags
        if harm_cats:
            for j, cat in enumerate(cat_names):
                if cat in harm_cats:
                    cat_hit = any(kw in text for kw in harm_cats[cat])
                    features[i, 6 + j] = 1.0 if cat_hit else 0.0
...
def train_tfidf(texts, labels):
    ...
    harm_kws, harm_cats = load_harm_intent_keywords()
    ...
    try:
        from pythainlp.tokenize import word_tokenize
        print(f"\n[3/4] Training TF-IDF + Keywords + Harm-Intent + LR...")
    ...
    # Keyword features (13 features: toxic + harm intent + per-category flags)
    all_kws = keywords + harm_kws
    if all_kws:
        print(f"  Adding keyword features ({len(keywords)} toxic + {len(harm_kws)} harm-intent)...")
        X_train_kw = keyword_features(train_texts, keywords, harm_kws, harm_cats)
        X_test_kw = keyword_features(test_texts, keywords, harm_kws, harm_cats)
    ...
    # Save harm intent keywords
    hi_path = MODEL_DIR / "harm_intent_keywords.json"
    with open(str(hi_path), "w", encoding="utf-8") as f:
        json.dump({"all": harm_kws, "categories": harm_cats}, f, ensure_ascii=False, indent=2)
        f.flush()
        os.fsync(f.fileno())
    ...
    config = {
        ...
        "harm_intent_count": len(harm_kws),
